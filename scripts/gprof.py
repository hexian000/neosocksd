...
def remove_stale_profile_data(profile_build_dir: Path) -> int:
	removed = 0
	for pattern in ("gmon.out", "gmon.out.*", "proxy-gmon.*"):
...
def main(argv: Optional[Sequence[str]] = None) -> int:
	...
	binary_path = profile_build_dir / "bin" / "neosocksd"
	...
	try:
		if not args.skip_benchmark:
			...
			proxy_log = (profile_build_dir / "neosocksd.log").open("w", encoding="utf-8")
			...
			proxy_env["GMON_OUT_PREFIX"] = str(profile_build_dir / "proxy-gmon")
